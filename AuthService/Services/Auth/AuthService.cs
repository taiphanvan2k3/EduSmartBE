    ...
    : BaseService(serviceProvider, logger), IAuthService
{
    ...
    {
        AuthTokens authTokens = _tokenService.GenerateTokens(userInfo);
    ...
    public async Task<ResponseInfo> CheckLogin(LoginRequest loginRequest)
    {
        try
        {
            ...
            Microsoft.AspNetCore.Identity.SignInResult result = await _signInManager.PasswordSignInAsync(user.UserName, loginRequest.Password, isPersistent: false,
            ...
            UserInfo userInfo = await ConvertAppUserToUserInfo(user);
    ...
    public async Task<ResponseInfo> LoginGoogleByToken(GoogleLoginRequest googleLoginRequest, string refreshToken = null)
    {
        ...
        try
        {
            ...
            GoogleJsonWebSignature.Payload payLoad = await GoogleJsonWebSignature.ValidateAsync(googleLoginRequest.IdToken);
            ...
            else
            {
                ...
                bool isAuthenticatedByGoogle = await _userManager.FindByLoginAsync("Google", payLoad.Subject) != null;
    ...
    public async Task<ResponseInfo> LoginGoogleByCode(GoogleLoginRequest googleLoginRequest)
    {
        ...
        try
        {
            ...
            ResponseInfo responseInfo = await LoginGoogleByToken(googleLoginRequest, googleTokenResponse.RefreshToken);
            _logger.LogInformation("[AuthService][{MethodName}] End", methodName);
            return responseInfo;
        }
        catch (Exception e)
    ...
    public async Task<ResponseInfo> ExternalLogin(ExternalLoginRequest externalLoginRequest)
    {
        ...
        {
            var responseInfo = new ResponseInfo();
            ApplicationUser user = await _userManager.FindByEmailAsync(externalLoginRequest.UserInfo.Email);
            if (user == null)
            {
                responseInfo.StatusCode = StatusCodes.Status409Conflict;
                responseInfo.Error = "UserNotSignedUp";
            ...
            {
                ApplicationUser loginInfo = await _userManager.FindByLoginAsync(externalLoginRequest.Provider,
    ...
    public async Task<ResponseInfo> ForgotPassword(ForgotPasswordContent forgotPasswordContent)
    {
        try
        {
            ...
            ApplicationUser user = await _userManager.FindByEmailAsync(forgotPasswordContent.Email);
            if (user == null)
            {
            ...
            string cacheKey = CacheKeyManager.GetResetPasswordKey(user.Email);
            bool isOtpSaved = _cacheService.SetData(cacheKey, otpData, otpData.OtpExpiry);
            if (!isOtpSaved)
            {
    ...
    public ResponseInfo ValidateOtpCode(OtpType otpType, string email, string otpCode)
    {
        ...
        try
        {
            ...
            OtpWrapperBase otpData = otpWrapperType switch
            {
    ...
    public async Task<ResponseInfo> ResetPassword(ResetPasswordContent resetPasswordContent)
    {
        ...
        try
        {
            ...
            ApplicationUser user = await _userManager.FindByEmailAsync(resetPasswordContent.Email);
            if (user == null)
            {
                responseInfo.Error = "UserNotFound";
                responseInfo.StatusCode = StatusCodes.Status404NotFound;
            ...
            string cacheKey = CacheKeyManager.GetResetPasswordKey(user.Email);
            ResetPasswordWrapper otpData = _cacheService.GetData<ResetPasswordWrapper>(cacheKey);
            ...
            IdentityResult result = await _userManager.ResetPasswordAsync(user, otpData.ResetPasswordToken, resetPasswordContent.NewPassword);
    ...
    {
        ApplicationUser user = await _userManager.FindByIdAsync(userId: userId.ToString());
        UserInfo userCreatedEventData = await ConvertAppUserToUserInfo(user);
