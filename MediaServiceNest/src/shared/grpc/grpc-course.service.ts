...
export class GrpcCourseService implements OnModuleInit {
  ...
  checkStudentCompletedCourse(
    ...
    this.logger.log(`Calling CheckStudentCompletedCourse for courseId: ${courseId}, studentId: ${studentId}`);
    return new Promise((resolve, reject) => {
      this.client.CheckStudentCompletedCourse(
        { studentId, courseId },
        (err: Error | null, response: CheckStudentCompletedCourseResponse) => {
          if (err) {
